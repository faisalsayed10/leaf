				...
				<SidebarContent>
					...
					<Menu>
						{data?.length > 0 ? (
							...
							<MenuItem onClick={onOpen} icon={<Icon as={GrAdd} {...IconProps} />}>
								Create a List
							</MenuItem>
						)}
					</Menu>
				...
				<SidebarFooter>
					<Menu iconShape="round">
						{!loading && session?.user ? (
							<MenuItem icon={<Icon as={BiUser} {...IconProps} />}>{session.user.name}</MenuItem>
						) : loading ? (
							<MenuItem>Just a Second...</MenuItem>
						) : (
							<MenuItem onClick={() => router.push("/signin")} icon={<Login {...IconProps} />}>
								Log In
							</MenuItem>
						)}
					</Menu>
