...
const MoreOptionsMenu: React.FC<Props> = ({ data }) => {
	...
	useEffect(() => {
		setBookData({
			...
			description: (data as Book)?.description || (data as Item)?.volumeInfo?.description,
			authors: (data as Book)?.authors || (data as Item)?.volumeInfo?.authors,
			publishedDate: (data as Book)?.publishedDate || (data as Item)?.volumeInfo?.publishedDate,
			previewLink: (data as Book)?.previewLink || (data as Item)?.volumeInfo?.previewLink,
			imageLinks: (data as Book)?.imageLinks || (data as Item)?.volumeInfo?.imageLinks
		});
	}, [data]);
	...
	return (
		<>
			<Menu>
				...
				<MenuList>
					...
					{router.route.startsWith("/list") && (
						<>
							<MenuDivider />
							<MenuItem icon={<CgRemoveR size="18" />} onClick={removeBookFromList}>
			...
			<CreateListModal onClose={onCloseCreateList} isOpen={isOpenCreateList} book={bookData} />
		</>
	);
};
